...
// Funções para buscar nomes por ID com tratamento de erros
...
function atualizarBotoesFavorito() {
  ...
  botoesFavorito.forEach(botao => {
    ...
    if (titulo) {
      ...
      const espaconaveEncontrada = favoritos.find(fav => fav.name === nomeEspaconave);
...
// Função para montar e exibir modal de favoritos (MESMA DOS VEÍCULOS)
...
// Função para exibir modal com detalhes da espaçonave (MESMA ESTRUTURA DOS VEÍCULOS)
async function exibirModalEspaconave(espaconave) {
  ...
  // Cria o grid de detalhes (MESMA ESTRUTURA DOS VEÍCULOS)
  ...
  const specsInfo = [
    { label: 'Velocidade Atmosférica', value: formatarVelocidade(espaconave.max_atmosphering_speed) },
    { label: 'Classificação Hiperdrive', value: espaconave.hyperdrive_rating || "Desconhecido" },
    { label: 'Capacidade de Carga', value: `${formatarNumero(espaconave.cargo_capacity)} kg` },
  ...
  // Grupo 3: Filmes (com links para abrir modal diretamente)
  ...
  if (espaconave.films && espaconave.films.length > 0) {
    ...
    const filmesPromises = espaconave.films.map(async (filmeUrl, index) => {
      ...
      // REMOVIDO: target='_blank' para permitir navegação na mesma página
  ...
  // Grupo 4: Pilotos (com links para abrir modal diretamente)
  ...
  // Configurar o botão de favoritar no modal
  const btnFavorite = document.getElementById('btn-favorite');
  if (btnFavorite) {
    // Atualizar texto do botão
    if (isFavorito(espaconave)) {
      btnFavorite.innerHTML = '<i class="fas fa-heart"></i> Remover dos Favoritos';
      btnFavorite.classList.add('favoritado');
    } else {
      btnFavorite.innerHTML = '<i class="far fa-heart"></i> Favoritar';
      btnFavorite.classList.remove('favoritado');
    }
    
    // Remover event listeners anteriores
    const newBtn = btnFavorite.cloneNode(true);
    btnFavorite.parentNode.replaceChild(newBtn, btnFavorite);
    
    // Adicionar novo event listener
    newBtn.addEventListener('click', () => {
      toggleFavorito(espaconave);
      if (isFavorito(espaconave)) {
        newBtn.innerHTML = '<i class="fas fa-heart"></i> Remover dos Favoritos';
        newBtn.classList.add('favoritado');
      } else {
        newBtn.innerHTML = '<i class="far fa-heart"></i> Favoritar';
        newBtn.classList.remove('favoritado');
      }
    });
  }
...
// Função para criar e adicionar cards na tela (MESMA ESTRUTURA DOS VEÍCULOS)
function adicionaCards(listaEspaconaves) {
  ...
  listaEspaconaves.forEach((espaconave, index) => {
    ...
    const featureList = [
      ...
      { label: 'Comprimento', value: formatarComprimento(espaconave.length) },
      { label: 'Tripulação', value: formatarNumero(espaconave.crew) },
      { label: 'Velocidade Máx.', value: formatarVelocidade(espaconave.max_atmosphering_speed) }
    ];
...
function filtrarPorNome(lista, textoUsuario) {
  ...
  const listaFiltrada = lista.filter((espaconave) => {
    return espaconave.name.toLowerCase().includes(textoEmMinusculo) ||
           (espaconave.model && espaconave.model.toLowerCase().includes(textoEmMinusculo)) ||
           (espaconave.manufacturer && espaconave.manufacturer.toLowerCase().includes(textoEmMinusculo));
  });
...
document.addEventListener('DOMContentLoaded', async function () {
  try {
    // Carrega todas as espaçonaves
    const espaconaves = await buscarDados('starships');
